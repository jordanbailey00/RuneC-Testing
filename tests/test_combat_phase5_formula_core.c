...
static RcNpc add_formula_npc(void) {
    ...
    RcNpc npc = {.def_id = def_idx};
    memcpy(npc.stats, g_npc_defs[def_idx].stats, sizeof(npc.stats));
    return npc;
}
...
static void test_scaled_hit_chance_is_deterministic(void) {
    assert(rc_hit_chance_scaled(100, 50) == 7425);
...
static void test_melee_formula_uses_boosts_prayer_equipment_and_npc_defence(void) {
    ...
    RcNpc npc = add_formula_npc();

    assert(rc_player_effective_attack_level(&p) == 92);
    assert(rc_player_effective_strength_level(&p) == 109);
    assert(rc_player_offensive_roll(&p, COMBAT_MELEE_SLASH) == 15088);
    assert(rc_npc_defensive_roll(&npc, COMBAT_MELEE_SLASH) == 3136);
    assert(rc_player_max_hit_melee(&p) == 19);

    RcCombatCalc calc = rc_calc_melee(&p, &npc);
    assert(calc.attack_roll == 15088);
    assert(calc.defence_roll == 3136);
    assert(calc.max_hit == 19);
}
...
static void test_ranged_formula_uses_ranged_prayers_and_strength_bonus(void) {
    ...
    assert(rc_player_max_hit_ranged(&p) == 16);

    RcCombatCalc calc = rc_calc_ranged(&p, &npc, true);
    assert(calc.attack_roll == 15092);
    assert(calc.defence_roll == 3136);
    assert(calc.max_hit == 16);
}
...
static void test_magic_formula_uses_magic_prayer_damage_and_defence_blend(void) {
    ...
    g_rc_prayer_defs[RC_PRAYER_AUGURY].magic_defence = 25;
    ...
    assert(rc_player_effective_magic_attack_level(&p) == 109);
    assert(rc_player_effective_magic_defence_level(&p) == 93);
    assert(rc_player_offensive_roll(&p, COMBAT_MAGIC) == 14606);
    assert(rc_player_defensive_roll(&p, COMBAT_MAGIC) == 8742);
    assert(rc_player_max_hit_magic(&p, 20) == 22);
    p.attack_stance = RC_ATTACK_STANCE_DEFENSIVE_CAST;
    assert(rc_player_effective_defence_level(&p) == 58);
    assert(rc_player_effective_magic_defence_level(&p) == 93);
    assert(rc_player_effective_magic_attack_level(&p) == 109);
    p.attack_stance = RC_ATTACK_STANCE_CAST;

    RcCombatCalc calc = rc_calc_magic(&p, &npc, 20);
    assert(calc.attack_roll == 14606);
    assert(calc.defence_roll == 5056);
...
static void test_npc_formula_uses_style_stat_and_player_defence_roll(void) {
    ...
    assert(rc_npc_offensive_roll(&npc, COMBAT_MAGIC) == 5056);
    assert(rc_player_effective_magic_defence_level(&p) == 89);
    assert(rc_player_defensive_roll(&p, COMBAT_MAGIC) == 8366);

    RcCombatCalc calc = rc_calc_npc_attack_style(&npc, rc_npc_def_get(npc.def_id),
                                                &p, COMBAT_MAGIC);
