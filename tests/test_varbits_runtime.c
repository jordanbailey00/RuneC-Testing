...
#define EXPECTED_VARBIT_COUNT 18571
#define EXPECTED_VARP_COUNT 5546
...
int main(void) {
    ...
    assert(rc_load_varbits(VBIT_PATH) == EXPECTED_VARBIT_COUNT);
    assert(rc_load_varps(VARP_PATH) == EXPECTED_VARP_COUNT);
    ...
    assert(world != NULL);
    assert(g_rc_varbit_count == EXPECTED_VARBIT_COUNT);
    assert(g_rc_varp_count == EXPECTED_VARP_COUNT);
