...
- Verification:
  - `rm -rf data/sprites/ui && mkdir -p data/sprites/ui && python3 tools/cache_pipeline/export_sprites_modern.py --cache /home/joe/projects/runescape-rl-reference/current_fightcaves_demo/data/cache --output data/sprites/ui` passed with `232` exported sprite groups and `0` failed groups.
  - `cmake --build /tmp/runec_copy_viewer_validation_build --target rc-viewer` passed.
  - `timeout 20 env RC_VIEWER_EXIT_FRAMES=2 /tmp/runec_copy_viewer_validation_build/rc-viewer` passed from the repo root.
...
- Verification:
  - `python3 tools/cache_pipeline/export_sprites_modern.py --cache /home/joe/projects/runescape-rl-reference/current_fightcaves_demo/data/cache --output data/sprites/ui` passed, exporting `208` sprite groups; missing modern-only sprite IDs were reported but nonfatal.
  - `cmake --build /tmp/runec_copy_viewer_validation_build --target rc-viewer` passed.
  - `timeout 20 env RC_VIEWER_EXIT_FRAMES=2 /tmp/runec_copy_viewer_validation_build/rc-viewer` passed from the repo root.
...
- Upstream/downstream impacts: the viewer now depends on
  ...
  combat input.
- Verification:
  - `python3 tools/cache_pipeline/export_sprites_modern.py --cache /home/joe/projects/runescape-rl-reference/current_fightcaves_demo/data/cache --output data/sprites/ui` passed, exporting `197` sprite groups. Two optional `prayerglow` sprite IDs were absent from this local cache and are not required by the current renderer.
  - `cmake -S /home/joe/projects/RuneC_copy -B /tmp/runec_copy_viewer_validation_build -DCMAKE_BUILD_TYPE=Release && cmake --build /tmp/runec_copy_viewer_validation_build --target rc-viewer` passed.
...
- Change made: verified the provisional source notes in
  `/home/joe/Downloads/osrs_parity_source_data_notes.md` against raw
